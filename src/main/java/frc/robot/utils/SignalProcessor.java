...
public class SignalProcessor {
     ...
     */
    public static <T> Future<?> processAsync(Supplier<T> supplier, Consumer<T> processor, String logKey) {
     ...
     */
    public static <T, R> Future<R> processTransformAsync(Supplier<T> supplier, 
     ...
     */
    public static double lowPassFilter(double currentValue, double previousFilteredValue, double alpha) {
        return alpha * currentValue + (1 - alpha) * previousFilteredValue;
    }
    
    /**
     * Shuts down the executor service when the robot is disabled or program is ending.
     * Should be called during robot disable or when the program is shutting down.
     */
